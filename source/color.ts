...
const fm = Symbol("IS FORMAT CODE");

export function xtColor(text = 7, background = 0) {
	let text_color_code = color_names.white;
	let background_color_code = color_names.black;

	if (text) {
		if (typeof text == "string")
			text_color_code = color_names[text.toLowerCase()] !== undefined
			? color_names[text.toLowerCase()]
			: text_color_code;
		else if (typeof text == "number")
			text_color_code = parseInt(text) & 0xFF;
	}

	if (background) {
		if (typeof background == "string")
			background_color_code = color_names[background.toLowerCase()] !== undefined
			? color_names[background.toLowerCase()]
			: background_color_code;
		else if (typeof background == "number")
			background_color_code = parseInt(background) & 0xFF;
	}

	return `38;5;${text_color_code};48;5;${background_color_code}`;
}
